...
| `Platform`, renderer timers/clock, time-budget checks | Parser runtime bridge plus legacy graph facade scheduling | C13a parser runtime and C13b resolver runtime injected; C15 owns remaining scheduler policy |
...
C13a extraction status: body grammar, parser DTOs, field normalization and property-reference enumeration now live in `src/core/parser/metadata.ts`. The portable cooperative parser receives explicit clock/yield/lifetime capabilities and does not select a browser timer. `src/index/fieldParser.ts` remains the host compatibility facade for metadata merge, linkpath resolution and the historical renderer-thread cooperative signature. `GraphBuilder` retains `mergeFileMetadata` until C13c; `GraphIndex`/`main.ts`/`SectionExpansion` retain host link extraction until C18/C22/C20b respectively; `MetadataParser` and `SectionExpansion` retain the renderer runtime bridge.

C13b accepted extraction status: semantic relationship enums/roles and generic relation flags live in `src/core/graph/relations.ts`; evidence storage/precedence lives in `src/core/graph/evidence.ts`; role classification, full/pair resolution and explanation decisions live in `src/core/graph/resolver.ts`. The resolver receives explicit clock/yield/current-generation capabilities and has no browser default scheduler. Legacy evidence/resolver paths are compatibility facades; the resolver facade alone supplies the renderer clock/timer for the historical signature. Normalized source records still do **not** carry roles, directions, precedence or inverse declarations. C13c must bind explicit source entities/semantic paths into compiler pages/evidence and remains the owner of self/index exclusions and source materialization. Cache/body acquisition stays C16 and publication/coordination stay C14/C15.
