...
C11 defined the host-free input vocabulary that later checkpoints feed into the existing K-Plex semantic compiler. Production adoption is incremental: C12a now supplies structural facts and C12b supplies aggregate host-link plus configured ontology facts, while `GraphBuilder`, `RelationEvidence` and `RelationResolver` remain the authoritative compiler/resolver. C12c still owns the remaining source families and C13 owns compiler extraction. `GraphIndex`, the event coordinator in `main.ts`, search, cache and snapshot algorithms remain on their existing owners.
...
| property discovery | `recordDiscoveredFields()` / cooperative variant inspect frontmatter keys and inline occurrences | Original/normalized field names and surface, including non-ontology fields, without arbitrary values | `field-name`; compiler retains existing rebuild-count / patch-discovery policy | Remaining collector wiring after C12b; C13 compiler extraction |
