...
This is intentionally not a general graph API. `layout.ts`, `PlexGraph.tsx`, `PlexFilter.tsx`, `ContentPane.tsx`, Graph Lens/predicate evaluation, relationship explanation/evidence, and other navigation/search callers still consume the concrete legacy index or legacy graph pages. Neighborhood and evidence ports should be added only when a real later consumer is migrated; presentation-filtered `neighbours()` must not be mislabeled as raw semantic adjacency. C10 owns lazy property/predicate extraction and C17 owns search implementation extraction.
